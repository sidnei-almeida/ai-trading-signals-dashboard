...
/** Offline full-history simulation (no PPO calls) for initial chart. */
export function simulateStaticHistory(
  rows: MarketDataRow[],
  startingCash = INITIAL_BALANCE,
): {
  agent_history: number[];
  benchmark_history: number[];
  price_history: DashboardData["price_history"];
  finalPortfolio: PortfolioState;
} {
  if (rows.length === 0) {
    return {
      agent_history: [startingCash],
      benchmark_history: [startingCash],
      price_history: [],
      finalPortfolio: {
        cash: startingCash,
  ...
  const benchmarkShares = initBenchmarkShares(rows, startingCash);
  let portfolio = initPortfolioAtRow(rows[0], startingCash);
  const agent_history: number[] = [];
  const benchmark_history: number[] = [];
  const targetWeight = 1 / RL_TICKERS.length;

  for (let i = 0; i < rows.length; i++) {
    const row = rows[i];
    const prices = pricesAtRow(row);
    const pv = portfolioValue(portfolio.cash, portfolio.shares, prices);
    agent_history.push(pv);
    benchmark_history.push(benchmarkValueAtRow(benchmarkShares, row));

    if (i >= rows.length - 1) break;

    const targets = RL_TICKERS.map(() => targetWeight);
    portfolio = rebalanceToTargets(portfolio, prices, targets);
  }

  const last = rows[rows.length - 1];
  const lastPrices = pricesAtRow(last);
  portfolio.allocation = weightsFromPortfolio(
    portfolio.cash,
    portfolio.shares,
    lastPrices,
  );
